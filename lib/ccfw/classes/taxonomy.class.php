 ...
 *
 */
class CCFramework_Taxonomy extends CCFramework_Abstract {
	...
	public function __construct($options) {
		...
		$this->options = apply_filters('cc_taxonomy_options', $options);
	...
	public function add_taxonomy_fields() {
		...
		foreach ($this->options as $option) {
			...
			$get_taxonomy = cc_get_var('taxonomy');
	...
	public function render_taxonomy_form_fields($term) {
		...
		$cc_errors = get_transient('cc-taxonomy-transient');

		wp_nonce_field('cc-taxonomy', 'cc-taxonomy-nonce');

		echo '<div class="cc-framework cc-taxonomy cc-taxonomy-' . esc_attr($classname) . '-fields">';
		...
		foreach ($this->options as $option) {
			...
			if ($taxonomy == $option['taxonomy']) {
				...
				foreach ($option['fields'] as $field) {
					...
					echo cc_add_element($field, $elem_value, $option['id']);
	...
	public function save_taxonomy($term_id) {
		...
		if (wp_verify_nonce(cc_get_var('cc-taxonomy-nonce'), 'cc-taxonomy')) {

			$errors = array();
			$taxonomy = cc_get_var('taxonomy');
			...
			foreach ($this->options as $request_value) {
				...
				if ($taxonomy == $request_value['taxonomy']) {
						...
						foreach ($request_value['fields'] as $field) {
							...
							if (isset($field['type']) && isset($field['id'])) {
								...
								if (has_filter('cc_sanitize_' . $sanitize_type)) {
									$request[$field['id']] = apply_filters('cc_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields']);
								}

								// validate options
								if (isset($field['validate']) && has_filter('cc_validate_' . $field['validate'])) {

									$validate = apply_filters('cc_validate_' . $field['validate'], $field_value, $field, $request_value['fields']);
					...
					$request = apply_filters('cc_save_taxonomy', $request, $request_key, $term_id);
			...
			set_transient('cc-taxonomy-transient', $errors, 10);
