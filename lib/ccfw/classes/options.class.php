 ...
 *
 */
abstract class CCFramework_Options extends CCFramework_Abstract {
	...
	public function element_after() {
		...
		$out = (isset($this->field['info'])) ? '<p class="cc-text-desc">' . $this->field['info'] . '</p>' : '';
	...
	public function element_debug() {
		...
		if ((isset($this->field['debug']) && $this->field['debug'] === true) || (defined('CC_OPTIONS_DEBUG') && CC_OPTIONS_DEBUG)) {

			$value = $this->element_value();

			$out .= "<pre>";
			$out .= "<strong>" . __('CONFIG', 'cc-framework') . ":</strong>";
			$out .= "\n";
			ob_start();
			var_export($this->field);
			$out .= htmlspecialchars(ob_get_clean());
			$out .= "\n\n";
			$out .= "<strong>" . __('USAGE', 'cc-framework') . ":</strong>";
			$out .= "\n";
			$out .= (isset($this->field['id'])) ? "cc_get_option( '" . $this->field['id'] . "' );" : '';

			if (!empty($value)) {
				$out .= "\n\n";
				$out .= "<strong>" . __('VALUE', 'cc-framework') . ":</strong>";
				$out .= "\n";
				ob_start();
				var_export($value);
				$out .= htmlspecialchars(ob_get_clean());
		...
		if ((isset($this->field['debug_light']) && $this->field['debug_light'] === true) || (defined('CC_OPTIONS_DEBUG_LIGHT') && CC_OPTIONS_DEBUG_LIGHT)) {
			...
			$out .= (isset($this->field['id'])) ? "cc_get_option( '" . $this->field['id'] . "' );" : '';
			$out .= "\n";
			$out .= "<strong>" . __('ID', 'cc-framework') . ":</strong>";
			$out .= "\n";
	...
	public function element_get_error() {
		...
		if (!empty($cc_errors)) {
			foreach ($cc_errors as $key => $value) {
				if (isset($this->field['id']) && $value['code'] == $this->field['id']) {
					$out .= '<p class="cc-text-warning">' . $value['message'] . '</p>';
				}
			}
		}
	...
	public function element_help() {
		return (isset($this->field['help'])) ? '<span class="cc-help" data-title="' . $this->field['help'] . '"><span class="fa fa-question-circle"></span></span>' : '';
	}
		...
		if (is_array($this->multilang)) {
			...
			foreach ($this->multilang['languages'] as $key => $val) {
				...
				if ($key != $this->multilang['current']) {
					...
					$class = 'CCFramework_Option_' . $this->field['type'];
			...
			$out .= '</fieldset>';
			$out .= '<p class="cc-text-desc">' . sprintf(__('You are editing language: ( <strong>%s</strong> )', 'cc-framework'), $this->multilang['current']) . '</p>';
	...
	public function element_multilang() {
		return (isset($this->field['multilang'])) ? cc_language_defaults() : false;
	}

}

// load all of fields
cc_load_option_fields();
