 ...
 *
 */
class CCFramework_Customize extends CCFramework_Abstract {
	...
	public function __construct($options) {
		...
		$this->options = apply_filters('cc_customize_options', $options);

		do_action('cc_customize_options_config', $this->options);
	...
	public function customize_register($wp_customize) {
		...
		cc_locate_template('functions/customize.php');
		do_action('cc_customize_register', $wp_customize);
	...
	public function add_section($wp_customize, $value, $panel = false) {
		...
		foreach ($sections as $section) {
			...
			foreach ($section['settings'] as $setting) {
				...
				$setting_name = CC_CUSTOMIZE . '[' . $setting['name'] . ']';
				...
				$wp_customize->add_setting($setting_name,
					wp_parse_args($setting, array(
							...
							'sanitize_callback' => 'cc_sanitize_clean',
						)
					)
				);
