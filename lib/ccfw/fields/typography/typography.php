 ...
 *
 */
class CCFramework_Option_typography extends CCFramework_Options {
  ...
  public function output() {
    ...
    $default_variants = apply_filters( 'cc_websafe_fonts_variants', array(
      'regular',
      'italic',
      '700',
      '700italic',
    ...
    $websafe_fonts = apply_filters( 'cc_websafe_fonts', array(
      'Arial',
      'Arial Black',
      'Comic Sans MS',
      'Impact',
    ...
    $google_json   = cc_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
      ...
      echo '<label class="cc-typography-family">';
      echo '<select name="'. $this->element_name( '[family]' ) .'" class="'. $is_chosen . $chosen_rtl .'cc-typo-family" data-atts="family"'. $this->element_attributes() .'>';

      do_action( 'cc_typography_family', $family_value, $this );

      echo '<optgroup label="'. __( 'Web Safe Fonts', 'cc-framework' ) .'">';
      ...
      echo '<optgroup label="'. __( 'Google Fonts', 'cc-framework' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        echo '<label class="cc-typography-variant">';
        echo '<select name="'. $this->element_name( '[variant]' ) .'" class="'. $is_chosen . $chosen_rtl .'cc-typo-variant" data-atts="variant">';
      ...
      echo '<input type="text" name="'. $this->element_name( '[font]' ) .'" class="cc-typo-font hidden" data-atts="font" value="'. $value['font'] .'" />';

    } else {

      echo __( 'Error! Can not load json file.', 'cc-framework' );
